...
from gestion_satisfactory.utils.helpers.string_utils import display_name_to_table_name, table_name_to_display_name
...
def create_page(title: str) -> None:
    ...
    with st.sidebar:
        ...
        with st.expander("Create a new party", expanded=False):
        ...
        with st.expander("Delete party", expanded=False):
            with st.form("form_delete_party"):
                factory_planner_selected_to_delete_display = st.selectbox(
                    label="Select your party", options=list(dict_display_table.keys())
                )
                ok = False
                if st.checkbox(
                    'Write "delete" to confirm'
                ):  # st.button("Do you want to remove this area ?", key="remove_area_button"):
        ...
        factory_planner_selected_display = st.selectbox(
            label="Select your party", options=list(dict_display_table.keys())
        )
            ...
            with st.expander("Add Area", expanded=False):
                ...
                if st.button("Confirm", key="add_area_button") and new_area_name != "":
                    ...
                    df_factory_planner = pd.concat(
                        [df_factory_planner, pd.DataFrame(row, columns=df_factory_planner_columns)], ignore_index=True
                    )
            ...
            with st.expander(f"⚙️ Manage Area **{area_selected}** ⚙️", expanded=False):
                ...
                with col1:
                    ...
                    if st.button("Confirm") and new_factory_name != "":
                        row = [
                            [area_selected, str(new_factory_name)] + [np.nan] * (len(df_factory_planner_columns) - 2)
                        ]
                        df_factory_planner = pd.concat(
                            [df_factory_planner, pd.DataFrame(row, columns=df_factory_planner_columns)],
                            ignore_index=True,
                        )
                        save_df(factory_planner_selected, df_factory_planner)
                        st.rerun()
                with col2:
                    if st.checkbox("Rename area ?"):
                        confirmation = st.text_input(
                            label="New name:", placeholder=f'Name to replace "{area_selected}"', value=""
                        )
                        if confirmation != "":
            ...
            with st.container():
                ...
                if factory_choice["selected_rows"] is not None:
                    ...
                    with col2_factory:
                        ...
                        display_results_item(
                            df_factory_planner_factories[df_factory_planner_factories["factory"] == factory_selected],
                            df_items,
                        )
                    ...
                    with st.expander(f"⚙️ Manage Factory **{factory_selected}** ⚙️", expanded=False):
                        ...
                        with col1:
                            new_line_name = st.text_input(
                                "Add production line",
                                placeholder="Name of production line",
                                key=f"text_input_{factory_selected}",
                                value="",
                            )
                            if st.button("Confirm", key=f"button_confirm_{factory_selected}") and new_line_name != "":
                                row = [
                                    [area_selected, factory_selected, str(new_line_name)]
                                    + [np.nan] * (len(df_factory_planner_columns) - 3)
                                ]
                                df_factory_planner = pd.concat(
                                    [df_factory_planner, pd.DataFrame(row, columns=df_factory_planner_columns)],
                                    ignore_index=True,
                        ...
                        with col2:
                            if st.checkbox("Rename factory ?"):
                                confirmation = st.text_input(
                                    label="New name:", placeholder=f'Name to replace "{factory_selected}"', value=""
                                )
                                if confirmation != "":
                                    ...
                                    df_factory_planner = pd.concat(
                                        [df_factory_planner, df_factory_planner_factories], ignore_index=True
                                    )
                                    save_df(factory_planner_selected, df_factory_planner)
                                    st.rerun()
                        with col3:
                            if st.checkbox("Delete factory ?", key=f"delete_factory_{factory_selected}"):
                                ...
                                if confirmation == factory_selected:
                                    df_factory_planner = df_factory_planner[
                                        (df_factory_planner["area"] != area_selected)
                                        | (
                                            (df_factory_planner["area"] == area_selected)
                                            & (df_factory_planner["factory"] != factory_selected)
                                        )
                                    ]
                                    save_df(factory_planner_selected, df_factory_planner)
                                    st.rerun()
                    ...
                    with st.container():
                        ...
                        with col1_line:
                            try:
                                df_factory_planner_lines = df_factory_planner_factories[
                                    df_factory_planner_factories["factory"] == factory_selected
                                ]
                                ...
                                with st.expander(f"⚙️ Manage line **{line_selected}** ⚙️", expanded=False):
                                    if st.checkbox("Delete production line ?", key=f"delete_line_{line_selected}"):
                                        ...
                                        if confirmation == line_selected:
                                            df_factory_planner_to_delete = df_factory_planner_lines[
                                                df_factory_planner_lines["line"] == line_selected
                                            ]
                                            df_factory_planner.drop(
                                                list(df_factory_planner_to_delete.index), inplace=True
                                            )
                                            save_df(factory_planner_selected, df_factory_planner)
                                            st.rerun()
                                    if st.checkbox("Rename production line ?"):
                                        confirmation = st.text_input(
                                            label="New name:",
                                            placeholder=f'Name to replace "{line_selected}"',
                                            value="",
                                        )
                                        if confirmation != "":
                                            ...
                                            df_factory_planner = pd.concat(
                                                [df_factory_planner, df_factory_planner_lines], ignore_index=True
                                            )
                                            save_df(factory_planner_selected, df_factory_planner)
                                            st.rerun()
                        ...
                        if line_choice["selected_rows"] is not None:
                            ...
                            with col2_line:
                                ...
                                display_results_item(
                                    df_factory_planner_lines[df_factory_planner_lines["line"] == line_selected],
                                    df_items,
                                )

                                with st.expander("Edit lines", expanded=False):
                                    list_of_recipes = sorted(list(df_recipes["recipe_name"].unique()))
                                    df_line = df_factory_planner_lines[
                                        df_factory_planner_lines["line"] == line_selected
                                    ]
                                    df_line_custom = pd.DataFrame(
                                        df_line[["nb_building", "rate/overclock", "recipe_name"]]
                                    ...
                                    column_config = {
                                        ...
                                        "recipe_name": st.column_config.SelectboxColumn(
                                            label="Recipe",
                                ...
                                display_recipes_frame(
                                    df_recipes, df_items, df_buildings, recipe_vars, recipe_var_to_name
                                )
